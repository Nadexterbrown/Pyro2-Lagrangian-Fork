...
class SSPRK2Stepper:
    ...
    def _single_stage(self, mesh, state, bc, dt, tnow):
        ...
        for j in range(ny):
            ...
            for i in range(1, nx):
                rhoL = max(primx['rhoL'][j,i], 1e-30)
                rhoR = max(primx['rhoR'][j,i-1], 1e-30)
                uL   = primx['uL'][j,i]
                uR   = primx['uR'][j,i-1]
                pL   = max(primx['pL'][j,i], 1e-30)
                pR   = max(primx['pR'][j,i-1], 1e-30)
        ...
        for i in range(nx):
            ...
            for j in range(1, ny):
                rhoB = max(primy['rhoB'][j,i], 1e-30)
                rhoT = max(primy['rhoT'][j-1,i], 1e-30)
                vB   = primy['vB'][j,i]
                vT   = primy['vT'][j-1,i]
                pB   = max(primy['pB'][j,i], 1e-30)
                pT   = max(primy['pT'][j-1,i], 1e-30)
